<template>
  <div v-if="profile" class="mx-auto w-full max-w-4xl space-y-6">
    ...
    <section v-if="isOwner" class="glass-panel"><div class="mb-6 flex flex-col justify-between gap-2 border-b border-white/[.08] pb-4 sm:flex-row sm:items-center"><div><p class="eyebrow mb-2 text-amber-300">ACCOUNT SECURITY</p><h2 class="font-display text-xl font-extrabold text-white">Đổi mật khẩu</h2></div><p class="text-xs text-slate-500">Xác thực bằng mật khẩu hiện tại của bạn.</p></div><div class="grid grid-cols-1 gap-4 sm:grid-cols-3"><div><label class="mb-2 block text-[10px] font-extrabold uppercase tracking-[.16em] text-sky-300">Mật khẩu hiện tại</label><input v-model="currentPassword" type="password" autocomplete="current-password" class="glass-input" placeholder="Nhập mật khẩu hiện tại"></div><div><label class="mb-2 block text-[10px] font-extrabold uppercase tracking-[.16em] text-sky-300">Mật khẩu mới</label><input v-model="newPassword" type="password" autocomplete="new-password" class="glass-input" placeholder="Ít nhất 6 ký tự"></div><div><label class="mb-2 block text-[10px] font-extrabold uppercase tracking-[.16em] text-sky-300">Xác nhận mật khẩu mới</label><input v-model="confirmPassword" type="password" autocomplete="new-password" class="glass-input" placeholder="Nhập lại mật khẩu mới"></div></div><button @click="changePassword" class="glass-btn mt-6 sm:mx-0 sm:w-auto">Đổi mật khẩu</button></section>
  </div>
...
const changePassword = async () => {
  if (!currentPassword.value || !newPassword.value || !confirmPassword.value) {
    return addToast('Vui lòng nhập đầy đủ thông tin mật khẩu.', 'warning');
  }
  if (newPassword.value.length < 6) {
    return addToast('Mật khẩu mới phải có ít nhất 6 ký tự.', 'warning');
  }
  if (newPassword.value !== confirmPassword.value) {
    return addToast('Mật khẩu xác nhận không khớp.', 'warning');
  }

  try {
    await axios.put(`${API}/users/${route.params.id}/change-password`, {
    ...
    addToast(Array.isArray(message) ? message.join(', ') : (message || 'Không thể đổi mật khẩu.'), 'error');
  }
};
</script>
