<template>
  <div class="w-full max-w-5xl space-y-6">
    <div class="glass-panel" v-if="activeMatch">
      ...
      <button @click="checkin" class="glass-btn !bg-green-600 hover:!bg-green-500 mt-4" :disabled="loading || hasCheckedIn">{{ loading ? 'Đang quét GPS...' : (hasCheckedIn ? 'Đã điểm danh' : '📍 ĐIỂM DANH GPS') }}</button>
      <div class="mt-6 border-t border-white/20 pt-4" v-if="activeDetails.attendances?.length">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div v-for="a in activeDetails.attendances" :key="a.id" class="bg-white/5 p-2 rounded-lg flex items-center gap-2">
            <img :src="a.user.avatar" class="w-10 h-10 rounded-full object-cover">
            <div class="text-sm">
              <p class="font-bold">{{ a.user.name }}</p>
              <p :class="a.status === 'Đúng giờ' ? 'text-green-400' : 'text-yellow-400'">{{ a.status }} <span class="text-xs" v-if="a.delay_seconds > 0">{{ formatDelay(a.delay_seconds) }}</span></p>
            </div>
          </div>
        </div>
      </div>
